...
class ParserTest extends Specification {
  ...
  "If statements are parsed, I" in {
    val src = """do
                ...
                |in 0""".stripMargin
    val ast = DoExpr(List(
                  IfThenElseStmt(ConstBExpr(true),
                      List(StringWriteStmt("OK")),
                      List(StringWriteStmt("No")))),
                NumExpr(0))
    Parser(src).get must_== ast
  }

  "If statements are parsed, II" in {
    val src = """do
                |  if true
                |  then write "OK"
                |    write "No"
                ...
                |in 0""".stripMargin
    val ast = DoExpr(List(
                  IfThenStmt(ConstBExpr(true),
                      List(StringWriteStmt("OK"), StringWriteStmt("No")))),
                NumExpr(0))
    Parser(src).get must_== ast
  }
  
  "While statements are parsed" in {
    val src = """let
                |  var x = 3
                |in do
                |  while x > 0 begin
                ...
                |  end
                |in x""".stripMargin
    val ast = LetExpr(List(VarDecl("x", NumExpr(3))),
                DoExpr(List(
                    WhileStmt(RelBExpr(">", IdExpr("x"), NumExpr(0)),
                      List(WriteStmt(IdExpr("x")),
                           AssignStmt("x", BinOpExpr("-",
                             IdExpr("x"),
                             NumExpr(1)))))),
                  IdExpr("x")))
    Parser(src).get must_== ast
